...
def evaluate(model, file_name, val_loader, batch_transforms, val_metric, amp=False):
    # Model in eval mode
    model.eval()
    # Reset val metric
    val_metric.reset()
    ...
    df.to_csv(f'./../results/{file_name}.csv', index=False, header=False, sep=' ')
...
def main(args):
    ...
    print(f"Test set loaded in {time.time() - st:.4}s ({len(train_set)} samples in " f"{len(test_loader)} batches)")
    ...
    val_loss, exact_match, partial_match = evaluate(model, args.name, test_loader, batch_transforms, val_metric, amp=args.amp)
